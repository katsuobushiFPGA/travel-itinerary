...
describe("parseMemberForm", () => {
  it("名前・役割・連絡先がすべて揃った場合に成功する", async () => {
    const fd = makeFormData({
      name: "山田太郎",
      role: "運転担当",
      contact: "090-1234-5678",
    });
    const result = await parseMemberForm(fd);
  ...
  it("名前のみでも成功し、role と contact は undefined になる", async () => {
  ...
  it("name がスペースのみの場合もエラーになる（trim 後に空）", async () => {
    const fd = makeFormData({ name: "   ", role: "", contact: "" });
    const result = await parseMemberForm(fd);
    expect(result.success).toBe(false);
  });

  it("role のみ空文字の場合は undefined に正規化される", async () => {
  ...
  it("contact のみ空文字の場合は undefined に正規化される", async () => {
  ...
  it("name が FormData に存在しない場合もエラーになる", async () => {
    const fd = new FormData();
    const result = await parseMemberForm(fd);
    expect(result.success).toBe(false);
  });
