...
describe("parsePackingForm", () => {
  it("正常系: name のみで quantity が 1 になる", async () => {
    const fd = makeFormData({ name: "歯ブラシ" });
    const result = await parsePackingForm(fd);
  ...
  it("category が空文字なら undefined に正規化される", async () => {
  ...
  it("quantity の文字列 '3' は数値 3 に coerce される", async () => {
  ...
  it("owner が空文字なら undefined に正規化される", async () => {
  ...
  it("name が空白のみならエラー（trim 後に空）", async () => {
    const fd = makeFormData({ name: "   " });
    const result = await parsePackingForm(fd);
    expect(result.success).toBe(false);
  });

  it("quantity が負数ならエラー", async () => {
    const fd = makeFormData({ name: "タオル", quantity: "-1" });
    const result = await parsePackingForm(fd);
    expect(result.success).toBe(false);
  });

  it("すべてのフィールドを正しく受け付ける", async () => {
